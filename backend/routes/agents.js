...
// List all agents with derived metric counts for the cards.
router.get('/', (_req, res) => {
  const agents = db.prepare('SELECT * FROM agents ORDER BY rowid').all();
  const enriched = agents.map((a) => {
    const ep = db.prepare('SELECT COUNT(*) AS c FROM episodes WHERE agent_id = ?').get(a.id).c;
    const ent = db.prepare('SELECT COUNT(*) AS c FROM entities WHERE agent_id = ?').get(a.id).c;
    const find = db
      .prepare("SELECT COUNT(*) AS c FROM findings WHERE agent_id = ? AND status = 'pending'")
      .get(a.id).c;
...
router.post('/findings/:id/:action', (req, res) => {
  ...
  const info = db
    .prepare('UPDATE findings SET status = ?, approved_at = CURRENT_TIMESTAMP WHERE id = ?')
    .run(status, id);
...
router.get('/:id/chat', (req, res) => {
  ...
  const messages = db
    .prepare('SELECT role, content, created_at FROM chat_messages WHERE agent_id = ? ORDER BY created_at')
    .all(agentId);
  res.json({ messages });
});
...
router.post('/:id/chat', async (req, res) => {
  ...
  db.prepare('INSERT INTO chat_messages (agent_id, role, content) VALUES (?, ?, ?)').run(
    agentId,
    'user',
  ...
  const history = db
    .prepare(
      'SELECT role, content FROM chat_messages WHERE agent_id = ? ORDER BY created_at DESC LIMIT 20'
    )
    .all(agentId)
    .reverse();

  if (!hasApiKey()) {
    const reply = `(${agentId} is offline — set GEMINI_API_KEY to chat for real.) Noted: "${userMessage.trim().slice(0, 120)}".`;
    db.prepare('INSERT INTO chat_messages (agent_id, role, content) VALUES (?, ?, ?)').run(
      agentId,
      'assistant',
      reply
